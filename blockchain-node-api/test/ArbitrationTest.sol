...
contract ArbitrationTest {
    ...
    function testIncorrectWithdraws() public {
        ...
        Assert.isTrue(seller.callWithdraw(), "1 seller withdraw call should succeed");
        Assert.equal(address(seller).balance, 0, "1 seller should not receive funds before finalize");
        Assert.isTrue(winner.callWithdraw(), "1 winner withdraw call should succeed");
        Assert.equal(address(winner).balance, 0, "1 winner should not receive funds before finalize");

        Assert.isTrue(winner.callFinalize(), "winner finalize call should succeed");

        Assert.isTrue(winner.callWithdraw(), "2 winner withdraw call should succeed");
        Assert.equal(address(winner).balance, 0, "2 winner should not receive funds after finalize");
        Assert.isTrue(seller.callWithdraw(), "2 seller withdraw call should succeed");
        Assert.equal(address(seller).balance, 100, "2 seller should receive funds after finalize");
    }
